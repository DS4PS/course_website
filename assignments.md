...
Part of the reason the field of data science is becoming popular and lucrative is advanced in computing technologies that allows us to run powerful programs and to work with large datasets on our own personal machines. Open source software has ensured these programs are available to lots of people. But another reason is that data has become so ubiquitous, cheap, and valuable. Data is an essential ingredient in data science. 

Your skill level in data science can be measured by how quickly you can take a real-world problem, break it into parts, generate some theories about performance, find the appropriate data to test your theories, run your models, and report your results. A big part of this is your ability to explore and analyze your data, but the process of obtaining data is not a trivial step. Having knowledge about where to look for raw data sources, or how to process and augment your existing data, will help you be more effective as an analyst. 

This section in the course focuses on finding data and getting it into R. You can of course download datasets from standard sites like the US Census, then import whatever file type was generated by the export into R using a variety of packages and functions. You might also use an API.

API stands for "Application Programming Interface", which is computer science jargon for the protocols used for two applications to speak to each other. If you are using your mobile phone and you want to log into your bank using an app, an API will allow you to send your user credential and password to your bank, and will return information about your balances and transactions. APIs are structured ways of sending data back and fourth. It creates the protocols that allow the user to ask a question (e.g. what is my checking account balance?), and the application to send an answer. In computer science, a question is called a query. 

In some cases, organizations that host public datasets have created data APIs. In these instances the user asks for a specific dataset, and the app sends a file as a response. To see some examples of how this works, visit the [Data Science Toolkit](http://www.datasciencetoolkit.org/ site), and test out the examples to see what sorts of data you might access through APIs. In some cases you give some search parameters (such as a zip code), and it returns a new dataset (census data). In other cases, you send data (raw text), and the API sends you a processed version of the data (a sentiment score based upon words in the text). Thus APIs are used both to access new data sources, as well as to clean or process your current data as part of your project.
